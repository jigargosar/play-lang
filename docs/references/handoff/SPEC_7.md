...
- `#` as first non-space character of a line only
- No inline comments
...
btn.disabled = false
modal.open = true
items.filter fn(item) item.visible
```
...
- `fn` — always named, always has parens, always a declaration
- `do` — always anonymous, parens optional, always a value
- Last expression returned implicitly in both
- One-line or indented block body in both
